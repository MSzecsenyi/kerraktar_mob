    ...
    const updateUniqueItem = (
        ...
        setUniqueItems((prevData) => {
            ...
            newData[key] = {
                ...
                alt_name:
                    alt_name !== undefined ? alt_name : prevData[key].alt_name,
                    ...
                    uniqueItems.some(
                        (uItem) =>
                            !item.unique_items.some(
                                (originalUItem) =>
                                    originalUItem.uuid === uItem.uuid
                            ) ||
                            !item.unique_items.some(
                                (originalUItem) =>
                                    originalUItem.alt_name === uItem.alt_name
                            )
                    )) ||
                (isUnique &&
                    item.unique_items.some(
                        (originalUItem) =>
                            !uniqueItems.some(
                                (uItem) => originalUItem.uuid === uItem.uuid
                            ) ||
                            !uniqueItems.some(
                                (uItem) =>
                                    originalUItem.alt_name === uItem.alt_name
                    ...
                    <View style={styles.basicInfoContainer}>
                        ...
                        <View style={styles.rowContainer}>
                            <View style={styles.checboxAligner}>
                                <TouchableWithoutFeedback
                                    onPress={() => {
                                        amount > MAX_UNIQUE_ITEMS &&
                                            setAmount(MAX_UNIQUE_ITEMS);
                                        isUnique
                                            ? setMaxInput(999)
                                            : setMaxInput(MAX_UNIQUE_ITEMS);
